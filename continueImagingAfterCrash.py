...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 17 04:37:53 2016
...
def createDirs(dirname):
    ...
    try:
        ...
        os.mkdir(dirname+presentDate+'/')
        baseDir = dirname+presentDate+'/'
    except:
        try:
    ...
    try:
        ...
        os.mkdir(imDir)
        #create directory for saving captured images (to be deleted later) in the base directory
        #imDelDir=baseDir+'imDelData/'
        #os.mkdir(imDelDir)
...
def selPreviousRois(roiDirname):
    '''
    returns the ROIs by reading the ROI values from a previous ROI file
    '''
    try:
        tk.Tk().withdraw() # we don't want a full GUI, so keep the root window from appearing
        filename = tkd.askopenfilename(initialdir = roiDirname, filetypes=[("Text files","*.txt")]) # show an "Open" dialog box and return the path to the selected file
...
def getOldRois(dirname):
    '''
    returns the ROIs by reading the ROI values from a previous ROI file
    '''
    try:
        roiDir = dirname+"roi"
        filename = max(glob.iglob(roiDir+'/*.txt'), key=os.path.getctime)
        print "Using ROIs from :"+filename
        rois = np.genfromtxt(filename,dtype='uint16',delimiter=',')
        roi_l1 = rois[0,:]
        roi_r1 = rois[1,:]
...
def selROI2(imData):
    ...
    np.savetxt(roiFileName, roiVal, fmt='%d', delimiter = ',')
    logFileWrite("Selected new ROIs")
    ShowImageWithROI(imData, showRoiImage=False, saveRoiImage=True)
...
def getRoiVals(getROI=True, updateROI=False):
    '''
    Standalone function to select ROIs from an image captured by the camera
    '''
    #Initiate camera
...
newDir = '/media/pointgrey/data/'



oldDir = max(glob.glob(os.path.join(dirname, '*/')), key=os.path.getmtime)
...
try:
    baseDir, imDir, roiDir, csvDir = createDirs(newDir)
except:
...
print "\n\n\n\n-------Imaging Crashed, restarted automatically on "\
                +present_time()+"-------\n\n\n\n\n"
logFileWrite("\n\n\n\n-------Imaging Crashed, restarted automatically on "\
                +present_time()+"-------\n\n\n\n\n")

print  "Getting ROI from old Dir ("+oldDir+")"

roi_l1, roi_r1, roi_l2, roi_r2, roi_bg, roi_bg1 = getOldRois(oldDir)
...
getRoiVals(getROI=True, updateROI=False)
roiFileName = roiDir+"ROIs_"+present_time()+".txt"
np.savetxt(roiFileName, roiVal, fmt='%d', delimiter = ',')
os.chdir(imDir)
...
for nLoop in range (0,totalLoops):
