...
roiVal1 = np.array([504, 510, 110, 116],dtype=np.uint8)
...
#                selROI1(imData, getROI=False, updateROI=True, showRoiImage=False, saveRoiImage=True)
            ...
            print("\nCamera display started on %s"%present_time())
            logFileWrite("Camera display started on "+present_time())
            displayCam(c, im)
            print("Camera display exited on %s"%present_time())
...
#    if nLoop==100 and saveImData ==True:
...
#        logFileName = baseDir+"camloop.txt"
    ...
    try:
        ...
        CapNProc(c, im, nFrames, saveDir, baseDir, csvDir, saveImData, dirTime)
    except:
        saveImData = False
