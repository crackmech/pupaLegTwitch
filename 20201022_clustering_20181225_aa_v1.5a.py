...
def getClusteredData(args):
		...
		kmeans = KMeans(n_clusters= num_clusters, n_init = 12, max_iter = 300, random_state = 0, verbose=False, n_jobs=24).fit(windows)
...
nThreads = 6
winSizes = np.array([100], dtype=np.uint8)#np.arange(100,300,25)
nClusters = np.array([200], dtype=np.uint8)#np.arange(100,200,25)
...
pickleFileExt = '.pkl'
pickleFiles = getFiles(baseDir, ['*'+pickleFileExt])
print(pickleFiles)

for pklFile in pickleFiles:
    data = pickleOpen(pklFile)
#    clusData, winSize, nClus, dirString = pickleOpen(pklFile)
...
#print((data[2]))
#print((data[3]))
pltColors = [random_color(alpha=0.6) for _ in range(int(len(clusData)/5))]

clusters_count = np.array(sorted(clusData, key=len, reverse=True))

clus = clusters_count[10]
...
nRows = np.sqrt(len(clusters_count))
...
#    clusMean = np.mean(clus, axis=0)
#    clusSD = np.std(clus, axis=0)
#    plt.plot(clusMean, alpha = 0.5)
#    plt.xlim(0, winSize)
#    plt.ylim(0,50)
##    plt.fill_between(range(data[1]), clusMean-clusSD, clusMean+clusSD, alpha = 0.2)
#filename = dirString + os.sep + 'WL_' + str(winSize) + '_All_Twitches.png'
#plt.savefig(filename)
#plt.show()
...
#clusSD = np.std(clus, axis=0)
#plt.plot(clusMean)
#plt.fill_between(range(data[1]), clusMean-clusSD, clusMean+clusSD, alpha = 0.5)
#plt.show()
#
#for clus in clusters_count:
#    for clusData in clus:
#        plt.plot(clusData)
#    plt.show()
#plt.plot(clusters_count[0][0])
#plt.show()
...
#plt.plot(np.transpose(windowsPerCluster[idx]), linewidth=0.4, alpha=0.1)
...
#		print('Processed Leg {0} - window len. = {1}, num. clusters = {2}, onset margin = {3}'.format(i+1, window_length, nClusters, om))	
#	
#		windowsPerCluster = [[] for j in range(nClusters)]
#		for j in range(len(kmeans.labels_)):
#			windowsPerCluster[kmeans.labels_[j]].append(windows[i][j])
#		windowsPerCluster = np.asarray(windowsPerCluster)
#
#		cluster_centers = kmeans.cluster_centers_
#
#		for idx in range(len(windowsPerCluster)):
#			plt.plot(np.transpose(windowsPerCluster[idx]), linewidth=0.4, alpha=0.1)
#			plt.xlabel("Time window")
#			plt.ylabel("Distance (L2 on pixel differences)")
#			plt.title('All samples in cluster %i' %idx)
#			fName = 'Leg' + str(i) + '_allSamples_Cluster=' + str(idx) + '_WL=' + str(window_length) + '_NC=' + str(nClusters) + '_OM=' + str(om) + '.png'
#			pltSave = os.path.join(fly_dir, pltRoot, fName)
#			plt.title('All samples, Cluster=' + str(idx) + '_WL=' + str(window_length) = '_NC=' + str(nClusters) + '_OM=' + str(om)) 		
#			plt.savefig(pltSave)
#			#plt.show()
#			plt.gcf().clear()
#		
...
def getClusPlts(clusData, winSize, nClus, dirString):
    '''
    saves all plots based on the data in clusData
    '''
    #clusData, winSize, nClus, dirString = args
